...
# The six countries that are represented in the data are:
# - Chile
# - China
# - Germany
# - Mexico
...
# These barplots show us that even with wide differences in GDP, its effect on a countries life expectancy is potentially minimal. The only country that bucks this trend is Zimbabwe, but its lower life expectancy might be a symptom of more than just a lower GDP. Let's continue our investigation.
...
plt.savefig("le_violin.png")
plt.show()


# What do you notice about this distribution? Which country's life expactancy has changed the most?

# Zimbabwe by far has the the widest distribution of the six countries surveyed. This indicates that over the 15 year span that we have data, the life expectancy for Zimbabwe has increased the most. Whereas the other five countries values satyed relatively stagnant.
...
# - Both the United States and China experienced dramatic GDP growth, and Zimbabwe experienced a sharp rise in life expectancy
# - Starting at about 2005, that data suggests the greatest changes from then onward
# - Zimbabwe
# - Excluding Zimbabwe, the other five countries have roughly the same life expectancy
# - To a certain extent, a high GDP does correlate to a higher life expectancy
...
plt.savefig("le_scatter_facet.png")
plt.show()
...
# - China
# - Zimbabwe
# - No, really given the other plots that were made prior to this one
# - Not particularly
...
sns.set(style='whitegrid')
# Uncomment the code below and fill in the blanks
g2 = sns.FacetGrid(data=df, col="Country", col_wrap=3, height=4)
g2 = (g2.map(plt.plot, "Year", "LEABY", color="g").add_legend())
g2.fig.suptitle('Life Expectancy vs. Year per Country',fontsize=16)
g2.fig.subplots_adjust(top=0.90)
plt.savefig("le_line_facet.png")
plt.show()
...
# - Most countires, excluding Zimbabwe, are all wealthy, or at a latter stage of developing, resulting in higher life expectancies
...
g3 = sns.FacetGrid(data=df, col="Country", col_wrap=3, height=4)
g3 = (g3.map(plt.plot, "Year", "GDP", color="g").add_legend())
g3.fig.suptitle('GDP vs. Year per Country',fontsize=16)
g3.fig.subplots_adjust(top=0.90)
plt.savefig("gdp_line_facet.png")
plt.show()
...
#What trends can be observed with the removal of Zimbabwe

df_no_Zimbabwe = df[df["Country"] != "Zimbabwe"]
g4 = sns.FacetGrid(data=df_no_Zimbabwe, col="Country", col_wrap=3, height=4)
...
plt.savefig("nz_le_line_facet.png")
plt.show()
...
g5 = sns.FacetGrid(data=df_no_Zimbabwe, col="Country", col_wrap=3, height=4)
g5 = (g5.map(plt.plot, "Year", "GDP", color="r").add_legend())
g5.fig.suptitle('GDP vs. Year per Country',fontsize=16)
g5.fig.subplots_adjust(top=0.90)
plt.savefig("nz_gdp_line_facet.png")
plt.show()


# With the removal of Zimbabwe, the 2008 recession becomes more apparent in the data. While each country felt some effect of the recession, the United States experienced the most acute consequence with a dip in its total GDP. When it comes to life expectancy however, each of the five countries have fairly consistent upward trends with some noise from year to year. 
